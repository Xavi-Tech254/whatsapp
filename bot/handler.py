...
from models import db, User, Category, Payment, MpesaSMS, BotSession, Settings
...
def parse_mpesa(text):
    ...
    phone_match = re.search(r'from\s+(2547\d{8}|2541\d{8}|\+2547\d{8})', text, re.IGNORECASE)
...
def handle_message(number, text):
    ...
    mpesa_number = get_setting('mpesa_number', '0743810633')
    ...
    if sess.state == 'awaiting_payment' and sess.pending_category_id:
        if text.lower() in ['cancel', '0', 'back', 'menu'] or text == 'MAIN_MENU':
            sess.state = 'menu'
            sess.pending_category_id = None
            db.session.commit()
            return show_main_menu(user, bot_name, banner_url)

        cat = Category.query.get(sess.pending_category_id)
        if not cat:
            sess.state = 'menu'
            db.session.commit()
            return make_response("❌ Item not found. Send *menu* to start over.")

        parsed = parse_mpesa(text)
        if not parsed:
            return make_response(
                "❌ Could not read Mpesa message.\n\n"
                "Please paste the *full* Mpesa confirmation SMS.\n\n"
                f"💰 Amount: *Ksh {int(cat.price)}*\n"
                f"📱 To: *{mpesa_number}*\n\n"
                "Or send *cancel* to go back.",
                buttons=[{'id': 'MAIN_MENU', 'text': '❌ Cancel'}]
            )

        if Payment.query.filter_by(mpesa_code=parsed['code']).first():
            return make_response("❌ This Mpesa code has already been used.\nPlease make a *new payment*.")

        forwarded = MpesaSMS.query.filter_by(mpesa_code=parsed['code']).first()
        if not forwarded:
            return make_response(
                "⏳ Payment not yet received on our end.\n\n"
                f"Please wait a moment and try again, or ensure you sent to *{mpesa_number}*"
            )

        if forwarded.used:
            return make_response("❌ This Mpesa code has already been used.")

        if abs(forwarded.amount - cat.price) > 1:
            return make_response(
                f"❌ Wrong amount.\n"
                f"Expected: *Ksh {int(cat.price)}*\n"
                f"Received: *Ksh {int(forwarded.amount)}*\n\n"
                "Please pay the correct amount."
            )

        payment = Payment(
            user_id=user.id, category_id=cat.id,
            mpesa_code=parsed['code'], amount=forwarded.amount,
            phone_number=forwarded.sender_phone, verified=True
        )
        forwarded.used = True
        ...
        resp_text = f"✅ Payment confirmed! Thank you, *{user.username}*! 🎉\n\n📦 *{cat.icon} {cat.name}*\n\nHappy studying! 🎓"
        file_path = None
        if cat.file_path:
        ...
        return make_response(
            resp_text,
            buttons=[{'id': 'MAIN_MENU', 'text': '🏠 Main Menu'}],
            file_path=file_path
        )
    ...
    if sess.state in ['menu', 'browsing']:
        ...
        if text.isdigit() or text in [str(c.id) for c in cats]:
            ...
            sess.pending_category_id = selected.id
            db.session.commit()
            return make_response(
                f"📦 *{selected.icon} {selected.name}*\n"
                f"💰 Price: *Ksh {int(selected.price)}*\n\n"
                f"To pay:\n"
                f"1️⃣ Send *Ksh {int(selected.price)}* to *{mpesa_number}*\n"
                f"2️⃣ Paste your Mpesa confirmation SMS here\n\n"
